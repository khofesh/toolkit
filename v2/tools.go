	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				// look into the first 512 bytes of this file
...
// CreateDirIfNotExist creates a directory and all necessary parent
// if it doesn't exist
...
// Slugify is a simple of function to create a slug from a string
...
// ReadJSON tries to read the body of a request and converts it from json into go data var
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data any) error {
	maxBytes := 1024 * 1024 // 1 M
	if t.MaxJSONSize != 0 {
...
// ErrorJSON takes an error and a status code and generate & sends a JSON error message
